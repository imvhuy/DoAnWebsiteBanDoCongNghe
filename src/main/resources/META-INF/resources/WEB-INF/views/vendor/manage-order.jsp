        ...
        <div class="col-md-9">
            ...
            <form action="${pageContext.request.contextPath}/vendor/manage-orders" method="get" class="mb-4">
                <div class="input-group">
                    <input type="text" class="form-control" placeholder="Tìm kiếm đơn hàng" name="orderId"
                           value="${orderId}">
                        ...
                        <td>
                            <form action="${pageContext.request.contextPath}/vendor/update-order-status" method="post">
                                ...
                                <select class="form-select" name="newStatus" onchange="this.form.submit()">
                                    ...
                                    <option value="RETURNED" ${order.status == 'RETURNED' ? 'selected' : ''}>Trả Hàng/Hoàn
                                        Tiền
                                    </option>
                                </select>
                            </form>
            ...
            <nav aria-label="Page navigation">
                <ul class="pagination justify-content-center">
                    <c:if test="${currentPage > 1}">
                        <li class="page-item">
                            <a class="page-link"
                               href="${pageContext.request.contextPath}/vendor/manage-orders?page=${currentPage - 1}&status=${status}">Previous</a>
                        </li>
                    </c:if>
                    <c:forEach var="i" begin="1" end="${totalPages}">
                        <li class="page-item ${i == currentPage ? 'active' : ''}">
                            <a class="page-link"
                               href="${pageContext.request.contextPath}/vendor/manage-orders?page=${i}&status=${status}">${i}</a>
                        </li>
                    </c:forEach>
                    <c:if test="${currentPage < totalPages}">
                        <li class="page-item">
                            <a class="page-link"
                               href="${pageContext.request.contextPath}/vendor/manage-orders?page=${currentPage + 1}&status=${status}">Next</a>
                        </li>
                    </c:if>
                </ul>
            </nav>
